  ...
  io.on('connection', (socket) => {
    ...
    socket.on('start_session', async ({ sessionId, fingerprint }) => {
      try {
        ...
        // Instant greeting (no LLM — keeps session_started fast)
        const greeting = claudeService.getGreeting(
          isReturning,
          lead.name,
    ...
    socket.on('send_message', async ({ content }) => {
      try {
        ...
        // Re-score lead after every turn (full transcript includes user + assistant messages)
        ...
        if (scoreData.name) lead.name = scoreData.name;
        if (scoreData.email) lead.email = scoreData.email;
        if (scoreData.phone) lead.phone = scoreData.phone;
        if (scoreData.company) lead.company = scoreData.company;
