...
// ─── GREETING MESSAGE (sync — no API call; session start stays instant) ───────
function getGreeting(isReturning, leadName, previousConversations = []) {
  if (!isReturning) {
  ...
  const name = leadName && leadName !== 'Unknown' ? leadName : null;
  const intro = name ? `Welcome back, ${name}!` : 'Welcome back!';

  if (previousConversations && previousConversations.length > 0) {
    const last = previousConversations[0];
    let memory = '';
    if (last.quote?.priceMin != null && last.quote?.priceMax != null) {
      memory = ` Last time we shared an estimated range around $${Number(last.quote.priceMin).toLocaleString()} – $${Number(last.quote.priceMax).toLocaleString()}.`;
    } else {
      memory =
        ' Picking up from our last chat — happy to continue helping with your project.';
    }
    return `${intro} Great to see you again.${memory} What would you like to do today?`;
  }

  return `${intro} I'm Alex at BuildRight — how can I help you today?`;
}
